...
# MySQL主从同步如何保证数据一致性？
- 背景：高并发的场景，读远大于写，读写的压力都在一台主机上会造成很大压力
- 解决：
  - 半同步复制：主库在执行完客户端提交的事务后不是立刻返回给客户端，而是等待至少一个从库接收到并写到
  - 数据库中间件：
    - 写请求路由到主库，读请求路由到从库
    - 记录录所有路由到写库的key，在主从同步时间窗口内（假设是500ms），如果有读请求访问中间件，此时有可能从库还是旧数据，就把这个key上的读请求路由到主库
